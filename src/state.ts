...
export async function getRuntimeMeta(env: Env, key: string): Promise<{ value: string; updatedAt: string } | undefined> {
  await ensureStateSchema(env);
...
export async function claimVideo(env: Env, video: VideoRecord): Promise<ClaimResult> {
  ...
  const nextEntry: ManifestEntry = {
    ...
    summaryModel: env.SUMMARY_ENABLED === "true" ? env.SUMMARY_MODEL : undefined,
    durationSeconds,
    chunkSeconds,
    totalChunks,
    nextChunkIndex,
  ...
  const result = await env.QUEUE_DB.prepare(
    ...
    env.SUMMARY_ENABLED === "true" ? env.SUMMARY_MODEL : null,
    durationSeconds,
    chunkSeconds,
    totalChunks,
    nextChunkIndex,
