...
function renderDashboardView(env: Env, rows: DashboardRow[], manifest: Manifest): DashboardView {
  ...
  const renderCard = (row: DashboardRow): string => {
    ...
    const retry = row.group === "failed"
      ? `<button class="retry-control action-button" type="button" data-retry-video="${escapeHtml(video.id)}"><span class="button-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><path d="M20 6v5h-5M4 18v-5h5M6.1 8.3A7 7 0 0 1 18.7 7L20 11M4 13l1.3 4A7 7 0 0 0 17.9 15.7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span class="button-label" data-i18n="card.retryNow">${escapeHtml(copyText("card.retryNow"))}</span></button>`
      : "";
    ...
    return `<article class="video-card" data-video-id="${escapeHtml(video.id)}" data-group="${row.group}" data-status-key="${escapeHtml(row.statusKey || "")}" data-progress="${percent ?? ""}" data-search="${escapeHtml(`${video.title} ${video.id}`.toLowerCase())}">
...
export function renderDashboard(env: Env, rows: DashboardRow[], manifest: Manifest, lastScan?: { value: string; updatedAt: string }): string {
  ...
  const activityHtml = activity.map((item) => {
    const activityId = `${item.at}|${item.labelKey}|${item.detail}`;
    return `<div class="activity-item" data-activity-id="${escapeHtml(activityId)}"><span class="activity-dot ${item.tone}"></span><div><strong data-i18n="${item.labelKey}">${escapeHtml(copyText(item.labelKey))}</strong><span>${escapeHtml(item.detail)}</span></div><time>${escapeHtml(taipeiTime(item.at))}</time></div>`;
  ...
  .header-actions{gap:8px}.language-toggle{position:relative;width:38px;height:38px;min-height:38px;padding:0;display:grid;place-items:center;border:1px solid #27323b;border-radius:11px;background:#10161b;color:#98a7b3;cursor:pointer;transition:background .18s ease,border-color .18s ease,color .18s ease,transform .18s cubic-bezier(.2,.8,.2,1);box-shadow:none}.language-toggle:hover{background:#171e24;border-color:#3a4854;color:#eef3f6}.language-toggle:active{transform:scale(.94)}.language-toggle svg{width:17px;height:17px;transition:transform .32s cubic-bezier(.2,.8,.2,1),opacity .18s ease}.language-toggle.switching svg{transform:rotate(100deg) scale(.86);opacity:.72}.language-toggle::after{content:attr(data-tooltip);position:absolute;right:0;top:calc(100% + 9px);z-index:20;pointer-events:none;white-space:nowrap;padding:6px 8px;border:1px solid #27323b;border-radius:8px;background:#11171c;color:#c4ced6;font-size:10px;font-weight:600;opacity:0;transform:translateY(-3px) scale(.98);transition:opacity .14s ease,transform .14s ease;box-shadow:0 8px 24px rgba(0,0,0,.28)}.language-toggle:hover::after,.language-toggle:focus-visible::after{opacity:1;transform:translateY(0) scale(1)}.language-toggle:focus-visible,.icon-control:focus-visible,.control:focus-visible,.logout:focus-visible,.filter:focus-visible,.stat:focus-visible,.section-toggle:focus-visible,.retry-control:focus-visible{outline:2px solid #6db6ea;outline-offset:2px}.video-card{transition:border-color .18s ease,background .18s ease,transform .18s cubic-bezier(.2,.8,.2,1),box-shadow .18s ease}.video-card:hover{transform:translateY(-1px);box-shadow:0 12px 34px rgba(0,0,0,.14)}.attention-strip{transition:border-color .22s ease,background .22s ease,transform .22s ease}.sync-dot{transition:background .2s ease,box-shadow .2s ease}.sync-panel.sync-success .sync-dot{animation:syncPulse .52s ease}.stat b.count-pop{animation:countPop .24s cubic-bezier(.2,.8,.2,1)}.flow.content-updated{animation:contentRefresh .28s cubic-bezier(.2,.8,.2,1)}.section-body.archive-enter{animation:archiveEnter .2s cubic-bezier(.2,.8,.2,1)}@keyframes syncPulse{0%{transform:scale(1)}45%{transform:scale(1.55)}100%{transform:scale(1)}}@keyframes countPop{0%{transform:translateY(0);opacity:1}45%{transform:translateY(-2px);opacity:.7}100%{transform:translateY(0);opacity:1}}@keyframes contentRefresh{0%{opacity:.78;transform:translateY(3px)}100%{opacity:1;transform:none}}@keyframes archiveEnter{0%{opacity:0;transform:translateY(-5px)}100%{opacity:1;transform:none}}@media(max-width:620px){.language-toggle{width:36px;height:36px;min-height:36px}.language-toggle::after{display:none}}.action-button{display:inline-flex;align-items:center;justify-content:center;gap:7px}.button-icon{width:14px;height:14px;display:grid;place-items:center;flex:0 0 auto;transition:transform .22s cubic-bezier(.2,.8,.2,1),opacity .18s ease}.button-icon svg{width:100%;height:100%}.action-button.is-working .button-icon{animation:buttonSpin .72s linear infinite}.action-button.is-success .button-icon{animation:buttonSuccess .34s cubic-bezier(.2,.8,.2,1)}.button-label{transition:opacity .14s ease,transform .18s cubic-bezier(.2,.8,.2,1)}.action-button.is-working .button-label{opacity:.78}.status{transition:background .22s ease,border-color .22s ease,color .22s ease,transform .22s cubic-bezier(.2,.8,.2,1),opacity .18s ease}.status.status-morph{animation:statusMorph .32s cubic-bezier(.2,.8,.2,1)}.video-card.state-changed{animation:cardStateFlash .62s ease}.video-card.completed-now{animation:cardCompleteFlash .72s ease}.progress-fill{transition:width .36s cubic-bezier(.2,.8,.2,1)}.progress-row>span{font-variant-numeric:tabular-nums}.filter,.stat{will-change:transform}.activity-item.activity-new{animation:activityEnter .3s cubic-bezier(.2,.8,.2,1)}.attention-icon.attention-morph{animation:attentionMorph .3s cubic-bezier(.2,.8,.2,1)}.sync-panel.syncing .sync-dot{animation:syncBreath 1.45s ease-in-out infinite}.flow.motion-active .video-card{will-change:transform,opacity}.video-card.filter-enter{animation:filterEnter .2s cubic-bezier(.2,.8,.2,1)}@keyframes buttonSpin{to{transform:rotate(360deg)}}@keyframes buttonSuccess{0%{transform:scale(.7);opacity:.5}65%{transform:scale(1.16);opacity:1}100%{transform:scale(1)}}@keyframes statusMorph{0%{opacity:.45;transform:scale(.94)}100%{opacity:1;transform:scale(1)}}@keyframes cardStateFlash{0%{box-shadow:0 0 0 0 rgba(131,199,255,0);border-color:var(--line-soft)}35%{box-shadow:0 0 0 3px rgba(131,199,255,.06);border-color:#344958}100%{box-shadow:0 0 0 0 rgba(131,199,255,0)}}@keyframes cardCompleteFlash{0%{box-shadow:0 0 0 0 rgba(131,223,165,0)}35%{box-shadow:0 0 0 4px rgba(131,223,165,.07);border-color:#2b5137}100%{box-shadow:0 0 0 0 rgba(131,223,165,0)}}@keyframes activityEnter{0%{opacity:0;transform:translateY(-7px)}100%{opacity:1;transform:none}}@keyframes attentionMorph{0%{transform:scale(.78);opacity:.45}70%{transform:scale(1.08);opacity:1}100%{transform:scale(1)}}@keyframes syncBreath{0%,100%{transform:scale(1);box-shadow:0 0 0 4px rgba(131,223,165,.08)}50%{transform:scale(1.2);box-shadow:0 0 0 7px rgba(131,223,165,.035)}}@keyframes filterEnter{0%{opacity:0;transform:translateY(4px) scale(.995)}100%{opacity:1;transform:none}}@media(prefers-reduced-motion:reduce){.language-toggle svg,.video-card,.attention-strip,.sync-dot,.progress-fill,.status,.button-icon,.button-label{transition:none!important}.language-toggle.switching svg,.sync-panel.sync-success .sync-dot,.sync-panel.syncing .sync-dot,.stat b.count-pop,.flow.content-updated,.section-body.archive-enter,.action-button.is-working .button-icon,.action-button.is-success .button-icon,.status.status-morph,.video-card.state-changed,.video-card.completed-now,.activity-item.activity-new,.attention-icon.attention-morph,.video-card.filter-enter{animation:none!important}}

  </style></head><body><main class="shell"><header class="page-header"><div class="brand"><div class="eyebrow">AI Memory · Ingest</div><h1>Meeting Memory</h1><p class="subtitle" id="dashboard-subtitle" data-i18n="header.subtitle" data-i18n-count="${counts.all}">${escapeHtml(copyText("header.subtitle","en",{count:counts.all}))}</p></div><div class="header-actions"><div class="sync-panel"><span class="sync-dot" aria-hidden="true"></span><div class="sync-copy"><strong id="refresh-copy" data-i18n="header.auto">${escapeHtml(copyText("header.auto"))}</strong><span id="manifest-status" data-i18n="manifest.refresh" data-i18n-time="${escapeHtml(taipeiTime(manifest.updatedAt))}">${escapeHtml(copyText("manifest.refresh","en",{time:taipeiTime(manifest.updatedAt)}))}</span></div><button class="icon-control" id="refresh-now" type="button" aria-label="${escapeHtml(copyText("header.refreshTitle"))}" title="${escapeHtml(copyText("header.refreshTitle"))}"><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M20 6v5h-5M4 18v-5h5M6.1 8.3A7 7 0 0 1 18.7 7L20 11M4 13l1.3 4A7 7 0 0 0 17.9 15.7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></button></div><button class="control action-button" id="run-now" type="button"><span class="button-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><path d="M20 6v5h-5M4 18v-5h5M6.1 8.3A7 7 0 0 1 18.7 7L20 11M4 13l1.3 4A7 7 0 0 0 17.9 15.7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span class="button-label" data-i18n="header.scan">${escapeHtml(copyText("header.scan"))}</span></button><button class="language-toggle" id="language-toggle" type="button" aria-label="${escapeHtml(copyText("header.switchLanguage"))}" title="${escapeHtml(copyText("header.switchLanguage"))}" data-tooltip="${escapeHtml(copyText("header.switchLanguage"))}"><svg class="language-icon" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="8.5" stroke="currentColor" stroke-width="1.7"/><path d="M3.8 9h16.4M3.8 15h16.4M12 3.5c2.1 2.25 3.15 5.08 3.15 8.5S14.1 18.25 12 20.5C9.9 18.25 8.85 15.42 8.85 12S9.9 5.75 12 3.5Z" stroke="currentColor" stroke-width="1.55" stroke-linecap="round" stroke-linejoin="round"/></svg></button><form method="post" action="/dashboard/logout"><button class="logout" type="submit"><span data-i18n="header.signOut">${escapeHtml(copyText("header.signOut"))}</span></button></form></div><div class="run-status" id="run-status" aria-live="polite"></div></header>
  ...
  function setLanguage(next){language=next==='zh'?'zh':'en';localStorage.setItem(languageKey,language);languageToggle.classList.add('switching');applyLocale();if(!reducedMotion())shell.animate([{opacity:.94,transform:'translateY(1px)'},{opacity:1,transform:'none'}],{duration:190,easing:'cubic-bezier(.2,.8,.2,1)'});setTimeout(()=>languageToggle.classList.remove('switching'),320);}
  ...
  function toggleArchiveAnimated(){if(!archive)return;const body=archive.querySelector('.section-body');const opening=archive.classList.contains('collapsed');archiveOpen=opening;if(opening){syncArchive();if(body&&!reducedMotion()){body.classList.remove('archive-enter');void body.offsetWidth;body.classList.add('archive-enter');}}else if(body&&!reducedMotion()){const animation=body.animate([{opacity:1,transform:'translateY(0)'},{opacity:0,transform:'translateY(-4px)'}],{duration:135,easing:'ease-out'});animation.finished.then(()=>{syncArchive();saveView();}).catch(()=>syncArchive());return;}else syncArchive();saveView();}
  function captureVisibleLayout(){const map=new Map();cards.forEach(card=>{if(card.hidden)return;const id=card.dataset.videoId;if(!id)return;const rect=card.getBoundingClientRect();map.set(id,{rect,statusKey:card.dataset.statusKey||'',group:card.dataset.group||'',progress:Number(card.dataset.progress)});});return map;}
  function animateLayoutFrom(before,{stateAware=false}={}){if(reducedMotion())return;flow.classList.add('motion-active');cards.forEach(card=>{if(card.hidden)return;const id=card.dataset.videoId;const previous=id?before.get(id):null;const rect=card.getBoundingClientRect();if(previous){const dx=previous.rect.left-rect.left,dy=previous.rect.top-rect.top;if(Math.abs(dx)>.5||Math.abs(dy)>.5){card.animate([{transform:'translate('+dx+'px,'+dy+'px)',opacity:.86},{transform:'translate(0,0)',opacity:1}],{duration:260,easing:'cubic-bezier(.2,.8,.2,1)'});}if(stateAware&&(previous.statusKey!==(card.dataset.statusKey||'')||previous.group!==(card.dataset.group||''))){const status=card.querySelector('.status');status?.classList.add('status-morph');const completedNow=previous.group!=='action'&&card.dataset.group==='action';card.classList.add(completedNow?'completed-now':'state-changed');setTimeout(()=>{status?.classList.remove('status-morph');card.classList.remove('state-changed','completed-now');},760);}animateProgress(card,previous.progress);}else{card.animate([{opacity:0,transform:'translateY(7px) scale(.995)'},{opacity:1,transform:'none'}],{duration:220,easing:'cubic-bezier(.2,.8,.2,1)'});}});setTimeout(()=>flow.classList.remove('motion-active'),320);}
  function animateProgress(card,oldPercent){const target=Number(card.dataset.progress);if(!Number.isFinite(oldPercent)||!Number.isFinite(target)||oldPercent===target)return;const track=card.querySelector('.progress-track'),fill=track?.querySelector('.progress-fill'),value=track?.parentElement?.lastElementChild;if(!fill)return;fill.style.transition='none';fill.style.width=Math.max(0,Math.min(100,oldPercent))+'%';void fill.offsetWidth;fill.style.transition='';requestAnimationFrame(()=>{fill.style.width=Math.max(0,Math.min(100,target))+'%';});if(value&&value!==fill){const from=oldPercent,to=target,start=performance.now(),duration=350;const tick=now=>{const p=Math.min(1,(now-start)/duration),eased=1-Math.pow(1-p,3);value.textContent=Math.round(from+(to-from)*eased)+'%';if(p<1)requestAnimationFrame(tick);};requestAnimationFrame(tick);}}
  function apply(animate=false){const before=animate?captureVisibleLayout():null;const q=(search.value||'').trim().toLowerCase();cards.forEach(card=>{const group=card.dataset.group;const groupMatch=active==='all'||active===group||(active==='active'&&(group==='processing'||group==='waiting'));const searchMatch=!q||(card.dataset.search||'').includes(q);card.hidden=!(groupMatch&&searchMatch);});sections.forEach(section=>{const sectionCards=[...section.querySelectorAll('.video-card')];const visible=sectionCards.filter(card=>!card.hidden).length;const isAttention=section.dataset.section==='attention';const showEmpty=isAttention&&active==='all'&&!q&&sectionCards.length===0;section.hidden=visible===0&&!showEmpty;});filters.forEach(x=>x.classList.toggle('active',x.dataset.filter===active));statButtons.forEach(x=>x.classList.toggle('active',x.dataset.filterTarget===active));syncArchive();if(before)animateLayoutFrom(before);}
  ...
  function chooseFilter(next,clearSearch=false){active=next||'all';if(clearSearch)search.value='';if(active==='private')archiveOpen=true;apply(true);saveView();}
  function renderAttention(animate=false){const attention=lastManualFailures+lastCounts.action;attentionStrip.classList.toggle('warn',attention>0);const nextIcon=attention>0?'!':'✓';if(attentionIcon.textContent!==nextIcon||animate){attentionIcon.textContent=nextIcon;if(!reducedMotion()){attentionIcon.classList.remove('attention-morph');void attentionIcon.offsetWidth;attentionIcon.classList.add('attention-morph');setTimeout(()=>attentionIcon.classList.remove('attention-morph'),340);}}if(lastManualFailures>0&&lastCounts.action>0)attentionTitle.textContent=t('attention.both',{failures:lastManualFailures,completed:lastCounts.action});else if(lastManualFailures>0)attentionTitle.textContent=t('attention.failureOnly',{count:lastManualFailures});else if(lastCounts.action>0)attentionTitle.textContent=t('attention.actionOnly',{count:lastCounts.action});else attentionTitle.textContent=t('attention.none');if(attention>0)attentionDetail.textContent=t('attention.detailIssues');else if(lastCounts.active>0)attentionDetail.textContent=t('attention.detailActive',{count:lastCounts.active});else attentionDetail.textContent=t('attention.detailClean');}
  ...
  function activityId(item){return String(item.at||'')+'|'+String(item.labelKey||'')+'|'+String(item.detail||'');}
  function activityMarkup(items){return (items||[]).map(item=>'<div class="activity-item" data-activity-id="'+escapeText(activityId(item))+'"><span class="activity-dot '+item.tone+'"></span><div><strong data-i18n="'+escapeText(item.labelKey)+'">'+escapeText(t(item.labelKey))+'</strong><span>'+escapeText(item.detail)+'</span></div><time>'+formatTaipei(item.at)+'</time></div>').join('')||'<div class="activity-empty" data-i18n="activity.empty">'+escapeText(t('activity.empty'))+'</div>';}
  function updateActivity(items){const before=new Map([...activityList.querySelectorAll('.activity-item')].map(node=>[node.dataset.activityId,node.getBoundingClientRect()]));activityList.innerHTML=activityMarkup(items);if(reducedMotion())return;[...activityList.querySelectorAll('.activity-item')].forEach(node=>{const previous=before.get(node.dataset.activityId),rect=node.getBoundingClientRect();if(previous){const dy=previous.top-rect.top;if(Math.abs(dy)>.5)node.animate([{transform:'translateY('+dy+'px)'},{transform:'translateY(0)'}],{duration:240,easing:'cubic-bezier(.2,.8,.2,1)'});}else{node.classList.add('activity-new');setTimeout(()=>node.classList.remove('activity-new'),340);}});}
  ...
  function animateCount(node){if(reducedMotion())return;node.classList.remove('count-pop');void node.offsetWidth;node.classList.add('count-pop');}
  const REFRESH_ICON='<svg viewBox="0 0 24 24" fill="none"><path d="M20 6v5h-5M4 18v-5h5M6.1 8.3A7 7 0 0 1 18.7 7L20 11M4 13l1.3 4A7 7 0 0 0 17.9 15.7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  const CHECK_ICON='<svg viewBox="0 0 24 24" fill="none"><path d="m5 12.5 4.2 4.2L19 7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  function setActionButton(button,state,labelKey){const icon=button.querySelector('.button-icon'),label=button.querySelector('.button-label');button.classList.toggle('is-working',state==='working');button.classList.toggle('is-success',state==='success');if(icon)icon.innerHTML=state==='success'?CHECK_ICON:REFRESH_ICON;if(label){label.dataset.i18n=labelKey;applyElement(label);}}
  function updateStaticState(data){const previousAttention=lastManualFailures+lastCounts.action;lastCounts=data.counts;lastManualFailures=data.manualFailures||0;lastScan.textContent=formatTaipei(data.lastScan?.at);lastCompleted.textContent=formatTaipei(data.lastCompletedAt);lastSync.dataset.i18n='ops.justNow';applyElement(lastSync);updateActivity(data.activity);subtitle.dataset.i18nCount=String(data.counts.all);applyElement(subtitle);manifestStatus.dataset.i18nTime=data.manifestUpdatedAt;applyElement(manifestStatus);renderAttention(previousAttention!==(lastManualFailures+lastCounts.action));statButtons.forEach(button=>{const key=button.dataset.filterTarget;const value=key==='all'?data.counts.all:data.counts[key];const number=button.querySelector('b');if(number&&typeof value==='number'&&number.textContent!==String(value)){number.textContent=String(value);animateCount(number);}});}
  async function refreshDashboard(manual=false){if(refreshing)return refreshing;refreshButton.classList.add('spinning');syncPanel.classList.remove('offline','stale','sync-success');syncPanel.classList.add('syncing');refreshButton.disabled=true;refreshCopy.textContent=t(manual?'header.refreshing':'header.syncing');refreshing=(async()=>{try{const response=await fetch('/dashboard/data',{headers:{accept:'application/json'},cache:'no-store'});if(response.status===401){location.href='/dashboard';return;}const data=await response.json();if(!response.ok)throw new Error(data.error||('HTTP '+response.status));const before=data.revision!==flow.dataset.revision?captureVisibleLayout():null;updateStaticState(data);if(before){flow.innerHTML=data.sectionsHtml||'<div class="empty" data-i18n="empty.playlist">'+escapeText(t('empty.playlist'))+'</div>';flow.dataset.revision=data.revision;refreshDynamicRefs();applyLocale(flow);apply(false);animateLayoutFrom(before,{stateAware:true});}refreshCopy.textContent=t('header.synced');syncPanel.classList.remove('offline','stale','syncing');syncPanel.classList.add('sync-success');setTimeout(()=>syncPanel.classList.remove('sync-success'),560);setTimeout(()=>{if(!refreshing)refreshCopy.textContent=t('header.auto');},1400);}catch(error){refreshCopy.textContent=t(navigator.onLine?'header.syncUnavailable':'header.offline');syncPanel.classList.remove('syncing');syncPanel.classList.add(navigator.onLine?'stale':'offline');lastSync.dataset.i18n='ops.syncFailed';applyElement(lastSync);console.error('Dashboard refresh failed',error);}finally{syncPanel.classList.remove('syncing');refreshButton.classList.remove('spinning');refreshButton.disabled=false;refreshing=null;}})();return refreshing;}
  refreshDynamicRefs();applyLocale();apply(false);requestAnimationFrame(()=>{if(Number.isFinite(Number(saved.scrollY)))window.scrollTo(0,Number(saved.scrollY)||0);});
  filters.forEach(button=>button.addEventListener('click',()=>chooseFilter(button.dataset.filter||'all')));statButtons.forEach(button=>button.addEventListener('click',()=>chooseFilter(button.dataset.filterTarget||'all',true)));search.addEventListener('input',()=>{apply(true);saveView();});flow.addEventListener('click',async event=>{const retry=event.target.closest?.('[data-retry-video]');if(retry){const videoId=retry.dataset.retryVideo;if(!videoId||retry.disabled)return;retry.disabled=true;setActionButton(retry,'working','card.retrying');try{const response=await fetch('/dashboard/retry/'+encodeURIComponent(videoId),{method:'POST',headers:{'x-dashboard-action':'retry'}});const data=await response.json();if(!response.ok)throw new Error(data.error||('HTTP '+response.status));setActionButton(retry,'success','feedback.retryButtonDone');runStatus.classList.remove('error');runStatus.textContent=t('feedback.retryButtonDone');await refreshDashboard(true);}catch(error){runStatus.classList.add('error');runStatus.textContent=t('feedback.retryFailed',{error:error instanceof Error?error.message:String(error)});retry.disabled=false;setActionButton(retry,'idle','card.retryNow');}return;}const toggle=event.target.closest?.('.section-toggle');if(!toggle||!archive?.contains(toggle))return;toggleArchiveAnimated();});window.addEventListener('pagehide',saveView);
  languageToggle.addEventListener('click',()=>setLanguage(language==='en'?'zh':'en'));refreshButton.addEventListener('click',()=>{void refreshDashboard(true);});setInterval(()=>{if(!document.hidden)void refreshDashboard(false);},60000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)void refreshDashboard(false);});window.addEventListener('online',()=>{void refreshDashboard(false);});window.addEventListener('offline',()=>{refreshCopy.textContent=t('header.offline');syncPanel.classList.remove('syncing');syncPanel.classList.add('offline');});setInterval(updateRelativeTimes,30000);
  runButton.addEventListener('click',async()=>{runButton.disabled=true;runStatus.classList.remove('error');runStatus.textContent=t('feedback.scanning');setActionButton(runButton,'working','feedback.scanning');try{const response=await fetch('/dashboard/run',{method:'POST',headers:{'x-dashboard-action':'run'}});const data=await response.json();if(!response.ok)throw new Error(data.error||('HTTP '+response.status));runStatus.textContent=t('feedback.scanComplete',{eligible:data.eligible,claimed:data.claimed});setActionButton(runButton,'success','feedback.scanButtonDone');await refreshDashboard(false);setTimeout(()=>{setActionButton(runButton,'idle','header.scan');runButton.disabled=false;},900);}catch(error){runStatus.classList.add('error');runStatus.textContent=t('feedback.scanFailed',{error:error instanceof Error?error.message:String(error)});setActionButton(runButton,'idle','header.scan');runButton.disabled=false;}});
  </script></body></html>`;
}
