...
export function renderDashboard(env: Env, rows: DashboardRow[], manifest: Manifest, lastScan?: { value: string; updatedAt: string }): string {
  ...
  function animateLayoutFrom(before,{stateAware=false}={}){if(reducedMotion())return;flow.classList.add('motion-active');cards.forEach(card=>{if(card.hidden)return;const id=card.dataset.videoId;const previous=id?before.get(id):null;const rect=card.getBoundingClientRect();if(previous){const dx=previous.rect.left-rect.left,dy=previous.rect.top-rect.top;if(Math.abs(dx)>.5||Math.abs(dy)>.5){card.animate([{transform:'translate('+dx+'px,'+dy+'px)',opacity:.86},{transform:'translate(0,0)',opacity:1}],{duration:260,easing:'cubic-bezier(.2,.8,.2,1)'});}if(stateAware&&(previous.statusKey!==(card.dataset.statusKey||'')||previous.group!==(card.dataset.group||''))){const status=card.querySelector('.status');status?.classList.add('status-morph');card.classList.add('state-changed');setTimeout(()=>{status?.classList.remove('status-morph');card.classList.remove('state-changed');},680);}animateProgress(card,previous.progress);}else{card.animate([{opacity:0,transform:'translateY(7px) scale(.995)'},{opacity:1,transform:'none'}],{duration:220,easing:'cubic-bezier(.2,.8,.2,1)'});}});setTimeout(()=>flow.classList.remove('motion-active'),320);}
