...
export function renderDashboard(env: Env, rows: DashboardRow[], manifest: Manifest, lastScan?: { value: string; updatedAt: string }): string {
  ...
  const activityHtml = activity.map((item) =>
    `<div class="activity-item" data-activity-key="${escapeHtml(`${item.at}|${item.labelKey}|${item.detail}`)}"><span class="activity-dot ${item.tone}"></span><div><strong data-i18n="${item.labelKey}">${escapeHtml(copyText(item.labelKey))}</strong><span>${escapeHtml(item.detail)}</span></div><time>${escapeHtml(taipeiTime(item.at))}</time></div>`,
  ...
  .motion-button{display:inline-flex;align-items:center;justify-content:center;gap:7px;position:relative}.motion-icon{width:16px;height:16px;display:inline-grid;place-items:center;flex:0 0 auto;transition:opacity .16s ease,transform .2s cubic-bezier(.2,.8,.2,1)}.motion-icon svg{width:100%;height:100%}.motion-button .icon-busy,.motion-button .icon-success{position:absolute;left:14px;opacity:0;transform:scale(.72)}.retry-control.motion-button .icon-busy,.retry-control.motion-button .icon-success{left:9px}.motion-button.busy .icon-default,.motion-button.success .icon-default{opacity:0;transform:scale(.72)}.motion-button.busy .icon-busy,.motion-button.success .icon-success{opacity:1;transform:scale(1)}.motion-button.busy .icon-busy svg{animation:buttonSpin .72s linear infinite}.motion-label{transition:opacity .16s ease,transform .18s ease}.motion-button.busy .motion-label{opacity:.72}.scan-control.success{border-color:#bde9cb;background:#e8f7ed}.progress-fill{transition:width .35s cubic-bezier(.2,.8,.2,1)}.progress-value{font-variant-numeric:tabular-nums;min-width:29px;text-align:right}.status{transition:background .2s ease,border-color .2s ease,color .2s ease,transform .2s cubic-bezier(.2,.8,.2,1),opacity .16s ease}.status.status-morph{animation:statusMorph .22s cubic-bezier(.2,.8,.2,1)}.video-card.state-changed{animation:stateEdge .42s ease}.video-card.completed-now{animation:completionEdge .58s ease}.video-card.card-enter{animation:cardEnter .24s cubic-bezier(.2,.8,.2,1)}.attention-strip.attention-changed{animation:attentionShift .28s cubic-bezier(.2,.8,.2,1)}.attention-icon.icon-changed{animation:attentionIcon .24s cubic-bezier(.2,.8,.2,1)}.activity-item.activity-new{animation:activityEnter .26s cubic-bezier(.2,.8,.2,1)}.sync-panel.syncing .sync-dot{animation:syncBreath 1.25s ease-in-out infinite}.filter,.stat{transition:background .16s ease,border-color .16s ease,color .16s ease,transform .16s ease,opacity .16s ease}@keyframes buttonSpin{to{transform:rotate(360deg)}}@keyframes statusMorph{0%{opacity:.45;transform:scale(.9)}65%{transform:scale(1.04)}100%{opacity:1;transform:scale(1)}}@keyframes stateEdge{0%{box-shadow:0 8px 28px rgba(0,0,0,.09)}35%{box-shadow:0 0 0 1px rgba(131,199,255,.22),0 13px 34px rgba(0,0,0,.16)}100%{box-shadow:0 8px 28px rgba(0,0,0,.09)}}@keyframes completionEdge{0%{box-shadow:0 8px 28px rgba(0,0,0,.09)}38%{box-shadow:0 0 0 2px rgba(131,223,165,.18),0 14px 36px rgba(0,0,0,.16);border-color:#31543d}100%{box-shadow:0 8px 28px rgba(0,0,0,.09)}}@keyframes cardEnter{0%{opacity:0;transform:translateY(6px) scale(.995)}100%{opacity:1;transform:none}}@keyframes attentionShift{0%{transform:scale(.996);opacity:.8}100%{transform:none;opacity:1}}@keyframes attentionIcon{0%{transform:scale(.72);opacity:.35}70%{transform:scale(1.1)}100%{transform:scale(1);opacity:1}}@keyframes activityEnter{0%{opacity:0;transform:translateY(-6px)}100%{opacity:1;transform:none}}@keyframes syncBreath{0%,100%{transform:scale(1);opacity:.72}50%{transform:scale(1.25);opacity:1}}::view-transition-group(*){animation-duration:.24s;animation-timing-function:cubic-bezier(.2,.8,.2,1)}::view-transition-old(*),::view-transition-new(*){animation-duration:.18s;animation-timing-function:ease-out}@media(prefers-reduced-motion:reduce){.motion-icon,.motion-label,.progress-fill,.status,.filter,.stat{transition:none!important}.motion-button.busy .icon-busy svg,.status.status-morph,.video-card.state-changed,.video-card.completed-now,.video-card.card-enter,.attention-strip.attention-changed,.attention-icon.icon-changed,.activity-item.activity-new,.sync-panel.syncing .sync-dot{animation:none!important}}
  ...
  function assignViewTransitionNames(){cards.forEach(card=>{const id=(card.dataset.videoId||'').replace(/[^A-Za-z0-9_-]/g,'');if(id)card.style.viewTransitionName='video-'+id;});sections.forEach(section=>{const id=(section.dataset.section||'').replace(/[^A-Za-z0-9_-]/g,'');if(id)section.style.viewTransitionName='section-'+id;});}
  function withViewTransition(update){if(typeof document.startViewTransition==='function'&&!reducedMotion()){try{return document.startViewTransition(update).finished.catch(()=>{});}catch{}}update();return Promise.resolve();}
  function captureCardMotionState(){const state=new Map();cards.forEach(card=>{if(card.hidden)return;const id=card.dataset.videoId;if(!id)return;const track=card.querySelector('[data-progress-percent]');state.set(id,{rect:card.getBoundingClientRect(),status:card.dataset.statusKey||'',group:card.dataset.group||'',percent:track?Number(track.dataset.progressPercent):undefined});});return state;}
  function animateProgressNumber(node,from,to){if(!node||!Number.isFinite(from)||!Number.isFinite(to)||from===to||reducedMotion()){if(node)node.textContent=Math.round(to)+'%';return;}const started=performance.now(),duration=350;const tick=now=>{const p=Math.min(1,(now-started)/duration),eased=1-Math.pow(1-p,3);node.textContent=Math.round(from+(to-from)*eased)+'%';if(p<1)requestAnimationFrame(tick);};requestAnimationFrame(tick);}
  function animateCardChanges(previous){cards.forEach(card=>{if(card.hidden)return;const id=card.dataset.videoId;if(!id)return;const before=previous.get(id),after=card.getBoundingClientRect();if(!reducedMotion()){if(before){const dx=before.rect.left-after.left,dy=before.rect.top-after.top;if(Math.abs(dx)>1||Math.abs(dy)>1)card.animate([{transform:'translate('+dx+'px,'+dy+'px)'},{transform:'translate(0,0)'}],{duration:260,easing:'cubic-bezier(.2,.8,.2,1)'});}else{card.classList.add('card-enter');setTimeout(()=>card.classList.remove('card-enter'),280);}}if(before&&before.status!==(card.dataset.statusKey||'')){const pill=card.querySelector('[data-status-pill]');if(pill&&!reducedMotion()){pill.classList.remove('status-morph');void pill.offsetWidth;pill.classList.add('status-morph');}const completedNow=before.group!=='action'&&card.dataset.group==='action';card.classList.add(completedNow?'completed-now':'state-changed');setTimeout(()=>card.classList.remove('state-changed','completed-now'),620);}const track=card.querySelector('[data-progress-percent]');if(track){const target=Number(track.dataset.progressPercent),fill=track.querySelector('.progress-fill'),value=card.querySelector('[data-progress-value]');const from=before&&Number.isFinite(before.percent)?before.percent:target;if(fill&&Number.isFinite(from)&&Number.isFinite(target)&&from!==target){fill.style.transition='none';fill.style.width=from+'%';void fill.offsetWidth;fill.style.transition='';requestAnimationFrame(()=>{fill.style.width=target+'%';});animateProgressNumber(value,from,target);}else if(value&&Number.isFinite(target))value.textContent=Math.round(target)+'%';}});}
  function setMotionButtonState(button,state){button.classList.toggle('busy',state==='busy');button.classList.toggle('success',state==='success');}
  function refreshDynamicRefs(){cards=[...flow.querySelectorAll('.video-card')];sections=[...flow.querySelectorAll('.flow-section')];archive=flow.querySelector('[data-section="archive"]');assignViewTransitionNames();}
  ...
  function chooseFilter(next,clearSearch=false){void withViewTransition(()=>{active=next||'all';if(clearSearch)search.value='';if(active==='private')archiveOpen=true;apply();saveView();});}
  function renderAttention(animate=true){const wasWarn=attentionStrip.classList.contains('warn'),oldTitle=attentionTitle.textContent||'',warn=lastManualFailures+lastCounts.action>0;attentionStrip.classList.toggle('warn',warn);attentionIcon.textContent=warn?'!':'✓';if(lastManualFailures>0&&lastCounts.action>0)attentionTitle.textContent=t('attention.both',{failures:lastManualFailures,completed:lastCounts.action});else if(lastManualFailures>0)attentionTitle.textContent=t('attention.failureOnly',{count:lastManualFailures});else if(lastCounts.action>0)attentionTitle.textContent=t('attention.actionOnly',{count:lastCounts.action});else attentionTitle.textContent=t('attention.none');if(warn)attentionDetail.textContent=t('attention.detailIssues');else if(lastCounts.active>0)attentionDetail.textContent=t('attention.detailActive',{count:lastCounts.active});else attentionDetail.textContent=t('attention.detailClean');if(animate&&!reducedMotion()&&(wasWarn!==warn||oldTitle!==attentionTitle.textContent)){attentionStrip.classList.remove('attention-changed');void attentionStrip.offsetWidth;attentionStrip.classList.add('attention-changed');if(wasWarn!==warn){attentionIcon.classList.remove('icon-changed');void attentionIcon.offsetWidth;attentionIcon.classList.add('icon-changed');}}}
  ...
  function updateStaticState(data){const oldActivity=new Map([...activityList.querySelectorAll('[data-activity-key]')].map(node=>[node.dataset.activityKey,node.getBoundingClientRect()]));lastCounts=data.counts;lastManualFailures=data.manualFailures||0;lastScan.textContent=formatTaipei(data.lastScan?.at);lastCompleted.textContent=formatTaipei(data.lastCompletedAt);lastSync.dataset.i18n='ops.justNow';applyElement(lastSync);activityList.innerHTML=activityMarkup(data.activity);activityList.querySelectorAll('[data-activity-key]').forEach(node=>{if(reducedMotion())return;const previous=oldActivity.get(node.dataset.activityKey);if(previous){const rect=node.getBoundingClientRect(),dy=previous.top-rect.top;if(Math.abs(dy)>1)node.animate([{transform:'translateY('+dy+'px)'},{transform:'translateY(0)'}],{duration:230,easing:'cubic-bezier(.2,.8,.2,1)'});}else{node.classList.add('activity-new');setTimeout(()=>node.classList.remove('activity-new'),320);}});subtitle.dataset.i18nCount=String(data.counts.all);applyElement(subtitle);manifestStatus.dataset.i18nTime=data.manifestUpdatedAt;applyElement(manifestStatus);renderAttention(true);statButtons.forEach(button=>{const key=button.dataset.filterTarget;const value=key==='all'?data.counts.all:data.counts[key];const number=button.querySelector('b');if(number&&typeof value==='number'&&number.textContent!==String(value)){number.textContent=String(value);animateCount(number);}});}
  async function refreshDashboard(manual=false){if(refreshing)return refreshing;refreshButton.classList.add('spinning');syncPanel.classList.remove('offline','stale','sync-success');syncPanel.classList.add('syncing');refreshButton.disabled=true;refreshCopy.textContent=t(manual?'header.refreshing':'header.syncing');refreshing=(async()=>{try{const response=await fetch('/dashboard/data',{headers:{accept:'application/json'},cache:'no-store'});if(response.status===401){location.href='/dashboard';return;}const data=await response.json();if(!response.ok)throw new Error(data.error||('HTTP '+response.status));const previous=data.revision!==flow.dataset.revision?captureCardMotionState():null;updateStaticState(data);if(previous){flow.innerHTML=data.sectionsHtml||'<div class="empty" data-i18n="empty.playlist">'+escapeText(t('empty.playlist'))+'</div>';flow.dataset.revision=data.revision;refreshDynamicRefs();applyLocale(flow);apply();animateCardChanges(previous);flow.classList.remove('content-updated');void flow.offsetWidth;flow.classList.add('content-updated');}refreshCopy.textContent=t('header.synced');syncPanel.classList.remove('offline','stale','syncing');syncPanel.classList.add('sync-success');setTimeout(()=>syncPanel.classList.remove('sync-success'),560);setTimeout(()=>{if(!refreshing)refreshCopy.textContent=t('header.auto');},1400);}catch(error){refreshCopy.textContent=t(navigator.onLine?'header.syncUnavailable':'header.offline');syncPanel.classList.remove('syncing');syncPanel.classList.add(navigator.onLine?'stale':'offline');lastSync.dataset.i18n='ops.syncFailed';applyElement(lastSync);console.error('Dashboard refresh failed',error);}finally{refreshButton.classList.remove('spinning');syncPanel.classList.remove('syncing');refreshButton.disabled=false;refreshing=null;}})();return refreshing;}
  ...
  filters.forEach(button=>button.addEventListener('click',()=>chooseFilter(button.dataset.filter||'all')));statButtons.forEach(button=>button.addEventListener('click',()=>chooseFilter(button.dataset.filterTarget||'all',true)));let searchMotionTimer;search.addEventListener('input',()=>{clearTimeout(searchMotionTimer);searchMotionTimer=setTimeout(()=>{void withViewTransition(()=>{apply();saveView();});},35);});flow.addEventListener('click',async event=>{const retry=event.target.closest?.('[data-retry-video]');if(retry){const videoId=retry.dataset.retryVideo;if(!videoId||retry.disabled)return;const label=retry.querySelector('.motion-label');retry.disabled=true;setMotionButtonState(retry,'busy');if(label){label.dataset.i18n='card.retrying';applyElement(label);}try{const response=await fetch('/dashboard/retry/'+encodeURIComponent(videoId),{method:'POST',headers:{'x-dashboard-action':'retry'}});const data=await response.json();if(!response.ok)throw new Error(data.error||('HTTP '+response.status));setMotionButtonState(retry,'success');await refreshDashboard(true);}catch(error){runStatus.classList.add('error');runStatus.textContent=t('feedback.retryFailed',{error:error instanceof Error?error.message:String(error)});retry.disabled=false;setMotionButtonState(retry,'idle');if(label){label.dataset.i18n='card.retryNow';applyElement(label);}}return;}const toggle=event.target.closest?.('.section-toggle');if(!toggle||!archive?.contains(toggle))return;toggleArchiveAnimated();});window.addEventListener('pagehide',saveView);
  ...
  runButton.addEventListener('click',async()=>{runButton.disabled=true;setMotionButtonState(runButton,'busy');runStatus.classList.remove('error');runStatus.textContent=t('feedback.scanning');let succeeded=false;try{const response=await fetch('/dashboard/run',{method:'POST',headers:{'x-dashboard-action':'run'}});const data=await response.json();if(!response.ok)throw new Error(data.error||('HTTP '+response.status));runStatus.textContent=t('feedback.scanComplete',{eligible:data.eligible,claimed:data.claimed});succeeded=true;setMotionButtonState(runButton,'success');await refreshDashboard(false);}catch(error){runStatus.classList.add('error');runStatus.textContent=t('feedback.scanFailed',{error:error instanceof Error?error.message:String(error)});}finally{if(succeeded&&!reducedMotion())setTimeout(()=>setMotionButtonState(runButton,'idle'),900);else setMotionButtonState(runButton,'idle');runButton.disabled=false;}});
  </script></body></html>`;
}
