...
function renderDashboard(env: Env, rows: DashboardRow[], manifest: Manifest, lastScan?: { value: string; updatedAt: string }): string {
  ...
  const translations=[
    ['監看轉錄、摘要與收尾狀態','Monitor transcription, summaries, and completion'],
    ['支影片',' videos'],['自動更新中','Auto-refreshing'],['秒更新','sec refresh'],['掃描 Playlist','Scan Playlist'],['登出','Sign out'],
    ['目前沒有需要你介入的項目','No action needed'],['目前沒有需要你處理的項目。','Nothing needs your attention.'],['目前沒有項目。','No items.'],
    ['目前流程是乾淨的，沒有異常或待收尾項目。','Pipeline is clear. No exceptions or cleanup needed.'],
    ['目前狀態','Current status'],['點數字即可篩選','Click a count to filter'],['已完成，可收回','Completed / cleanup'],['處理中 / 等待','Processing / waiting'],['失敗','Failed'],['待處理','Queued'],['目前可見','Visible'],
    ['搜尋標題或 video ID','Search title or video ID'],['全部','All'],['可收回','Cleanup'],['處理中','Active'],['最近事件','Recent activity'],['最近 8 筆','Latest 8'],
    ['需要處理','Needs attention'],['只有需要人工確認或收尾的項目會出現在這裡。','Only items requiring manual review or cleanup appear here.'],
    ['正在處理','In progress'],['轉錄、摘要與冷卻等待中的工作。','Transcription, summarization, and cooldown waits.'],
    ['已進 Playlist、等待自動 pipeline 接手。','In the playlist and waiting for the automatic pipeline.'],['其他影片','Other videos'],['Private 或目前不需要關注的項目，預設收起。','Private or low-attention items, collapsed by default.'],
    ['展開','Expand'],['收起','Collapse'],['建議','Action'],['進度','Progress'],['更新','Updated'],['Studio 編輯','Edit in Studio'],['錯誤資訊','Error details'],['立即重試','Retry now'],
    ['已完成','Completed'],['可移出 playlist','Can be removed from playlist'],['可改 Private 並移出 playlist','Set to Private and remove from playlist'],['轉錄中','Transcribing'],['不用操作','No action needed'],['整理摘要中','Finalizing'],['Cloudflare Workflow 正在摘要並發布','Workflow is summarizing and publishing'],
    ['YouTube 冷卻中','YouTube cooldown'],['下載出口被 YouTube 暫時阻擋；到時間後自動重試','YouTube temporarily blocked the download route; retry is automatic'],['等待 Groq 額度','Waiting for Groq quota'],['額度恢復後自動續跑','Resumes automatically when quota recovers'],
    ['YouTube 阻擋','YouTube blocked'],['下載出口受阻；系統會依失敗冷卻時間自動再試，也可立即重試','Download route blocked; automatic retry is scheduled, or retry now'],['達失敗冷卻時間後會自動再試；可展開錯誤資訊確認原因，或立即重試','Automatic retry follows the failure cooldown; inspect the error or retry now'],
    ['尚未排入處理','Not queued'],['Cron 會自動處理','Cron will process automatically'],['未追蹤','Untracked'],['目前 pipeline 只自動處理 Unlisted','The pipeline only processes Unlisted videos automatically'],
    ['完成','Complete'],['摘要 / 發佈','Summary / publish'],['啟動中','Starting'],['處理完成','Completed'],['處理失敗','Failed'],['等待自動重試','Waiting for auto-retry'],['YouTube 冷卻','YouTube cooldown'],['Groq 冷卻','Groq cooldown'],['開始整理摘要','Finalization started'],['開始處理','Processing started'],
    ['剛剛','Just now'],['還沒有近期事件。','No recent activity.'],['目前沒有可顯示的 playlist 影片。','No playlist videos to display.'],['更新中…','Refreshing…'],['同步中…','Syncing…'],['已同步','Synced'],['暫時無法同步','Sync unavailable'],['網路已離線','Offline'],['同步失敗','Sync failed'],['重試中…','Retrying…'],['重試失敗：','Retry failed: '],['正在掃描 Playlist…','Scanning playlist…'],['掃描完成：發現 ','Scan complete: found '],[' 支可處理，觸發 ',' eligible, started '],[' 支。',' videos.'],['觸發失敗：','Scan failed: '],
    ['輸入現有的 ADMIN_TOKEN 登入。Token 不會寫入網址或 localStorage。','Enter your existing ADMIN_TOKEN to sign in. The token is never stored in the URL or localStorage.'],['開啟 Dashboard','Open Dashboard'],['ADMIN_TOKEN 不正確。','Incorrect ADMIN_TOKEN.']
  ];
  function translateString(value){if(language==='zh')return value;let out=String(value??'');for(const [zh,en] of translations)out=out.split(zh).join(en);return out;}
  function translateTree(root=document.body){const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);nodes.forEach(node=>{if(!node.parentElement||['SCRIPT','STYLE','PRE'].includes(node.parentElement.tagName))return;if(!zhMap.has(node))zhMap.set(node,node.nodeValue||'');node.nodeValue=language==='zh'?zhMap.get(node):translateString(zhMap.get(node));});document.querySelectorAll('[placeholder]').forEach(el=>{if(!el.dataset.zhPlaceholder)el.dataset.zhPlaceholder=el.getAttribute('placeholder')||'';el.setAttribute('placeholder',language==='zh'?el.dataset.zhPlaceholder:translateString(el.dataset.zhPlaceholder));});document.documentElement.lang=language==='zh'?'zh-Hant':'en';languageToggle.textContent=language==='zh'?'English':'中文';}
  function setLanguage(next){language=next==='zh'?'zh':'en';localStorage.setItem(languageKey,language);translateTree();}
  ...
  function updateRelativeTimes(){const now=Date.now();document.querySelectorAll('.retry-time').forEach(node=>{const at=Date.parse(node.dataset.retryAt||'');if(!Number.isFinite(at))return;const minutes=Math.ceil((at-now)/60000);const verb=node.dataset.retryVerb||'重試';const absolute=new Intl.DateTimeFormat('zh-TW',{timeZone:'Asia/Taipei',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(at));node.textContent=language==='zh'?(minutes>1?minutes+' 分鐘後'+verb+' · '+absolute:minutes===1?'約 1 分鐘後'+verb+' · '+absolute:minutes===0?'即將'+verb+' · '+absolute:verb+'時間已到 · '+absolute):(minutes>1?verb+' in '+minutes+' min · '+absolute:minutes===1?verb+' in about 1 min · '+absolute:minutes===0?verb+' shortly · '+absolute:verb+' due now · '+absolute);});}
  function activityMarkup(items){return (items||[]).map(item=>'<div class="activity-item"><span class="activity-dot '+item.tone+'"></span><div><strong>'+escapeText(translateString(item.label))+'</strong><span>'+escapeText(item.detail)+'</span></div><time>'+formatTaipei(item.at)+'</time></div>').join('')||'<div class="activity-empty">還沒有近期事件。</div>';}
  ...
  function updateStaticState(data){lastScan.textContent=formatTaipei(data.lastScan?.at);lastCompleted.textContent=formatTaipei(data.lastCompletedAt);lastSync.textContent='剛剛';activityList.innerHTML=activityMarkup(data.activity);subtitle.textContent='監看轉錄、摘要與收尾狀態 · 目前可見 '+data.counts.all+' 支影片';manifestStatus.textContent='Manifest '+data.manifestUpdatedAt+' · 60 秒更新';attentionStrip.classList.toggle('warn',data.attentionCount>0);attentionIcon.textContent=data.attentionCount>0?'!':'✓';attentionTitle.textContent=translateString(data.attentionText);attentionDetail.textContent=translateString(data.attentionDetail);statButtons.forEach(button=>{const key=button.dataset.filterTarget;const value=key==='all'?data.counts.all:data.counts[key];const number=button.querySelector('b');if(number&&typeof value==='number')number.textContent=String(value);});}
  async function refreshDashboard(manual=false){if(refreshing)return refreshing;refreshButton.classList.add('spinning');syncPanel.classList.remove('offline','stale');refreshButton.disabled=true;refreshCopy.textContent=translateString(manual?'更新中…':'同步中…');refreshing=(async()=>{try{const response=await fetch('/dashboard/data',{headers:{accept:'application/json'},cache:'no-store'});if(response.status===401){location.href='/dashboard';return;}const data=await response.json();if(!response.ok)throw new Error(data.error||('HTTP '+response.status));updateStaticState(data);if(data.revision!==flow.dataset.revision){flow.innerHTML=data.sectionsHtml||'<div class="empty">目前沒有可顯示的 playlist 影片。</div>';flow.dataset.revision=data.revision;refreshDynamicRefs();translateTree(flow);apply();updateRelativeTimes();}refreshCopy.textContent=translateString('已同步');syncPanel.classList.remove('offline','stale');setTimeout(()=>{if(!refreshing)refreshCopy.textContent=translateString('自動更新中');},1400);}catch(error){refreshCopy.textContent=translateString(navigator.onLine?'暫時無法同步':'網路已離線');syncPanel.classList.add(navigator.onLine?'stale':'offline');lastSync.textContent=translateString('同步失敗');console.error('Dashboard refresh failed',error);}finally{refreshButton.classList.remove('spinning');refreshButton.disabled=false;refreshing=null;}})();return refreshing;}
  refreshDynamicRefs();translateTree();apply();updateRelativeTimes();requestAnimationFrame(()=>{if(Number.isFinite(Number(saved.scrollY)))window.scrollTo(0,Number(saved.scrollY)||0);});
  ...
  languageToggle.addEventListener('click',()=>{setLanguage(language==='en'?'zh':'en');updateRelativeTimes();});refreshButton.addEventListener('click',()=>{void refreshDashboard(true);});setInterval(()=>{if(!document.hidden)void refreshDashboard(false);},60000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)void refreshDashboard(false);});window.addEventListener('online',()=>{void refreshDashboard(false);});window.addEventListener('offline',()=>{refreshCopy.textContent=translateString('網路已離線');syncPanel.classList.add('offline');});setInterval(updateRelativeTimes,30000);
