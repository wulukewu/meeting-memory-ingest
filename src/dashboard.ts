...
function renderDashboardView(env: Env, rows: DashboardRow[], manifest: Manifest): DashboardView {
  ...
  const attentionCount = counts.action + counts.failed;
  ...
  const renderCard = (row: DashboardRow): string => {
    ...
    return `<article class="video-card" data-group="${row.group}" data-search="${escapeHtml(`${video.title} ${video.id}`.toLowerCase())}">
      ...
      ${error}
      <div class="card-footer"><div class="links"><a class="primary" href="${studio}" target="_blank" rel="noreferrer">Studio 編輯</a><a href="${playlist}" target="_blank" rel="noreferrer">Playlist</a><a href="${watch}" target="_blank" rel="noreferrer">YouTube</a>${transcript ? `<a href="${transcript}" target="_blank" rel="noreferrer">Transcript</a>` : ""}</div><span class="video-id" title="Video ID">${escapeHtml(video.id)}</span></div>
    </article>`;
  };
  ...
  const attentionText = attentionCount > 0
    ? `${counts.failed ? `${counts.failed} 個失敗` : ""}${counts.failed && counts.action ? " · " : ""}${counts.action ? `${counts.action} 支已完成待收回` : ""}`
    : "目前沒有需要你介入的項目";
  ...
  return { counts, attentionCount, attentionText, attentionDetail, sectionsHtml, revision };
}

async function loadDashboardState(env: Env): Promise<{ rows: DashboardRow[]; manifest: Manifest }> {
  ...
  return { rows, manifest: manifestResult.manifest };
}

function dashboardData(env: Env, rows: DashboardRow[], manifest: Manifest): Record<string, unknown> {
...
function renderDashboard(env: Env, rows: DashboardRow[], manifest: Manifest): string {
  ...
  function updateStaticState(data){subtitle.textContent='監看轉錄、摘要與收尾狀態 · 目前可見 '+data.counts.all+' 支影片';manifestStatus.textContent='Manifest '+data.manifestUpdatedAt+' · 60 秒更新';attentionStrip.classList.toggle('warn',data.attentionCount>0);attentionIcon.textContent=data.attentionCount>0?'!':'✓';attentionTitle.textContent=data.attentionText;attentionDetail.textContent=data.attentionDetail;statButtons.forEach(button=>{const key=button.dataset.filterTarget;const value=key==='all'?data.counts.all:data.counts[key];const number=button.querySelector('b');if(number&&typeof value==='number')number.textContent=String(value);});}
  async function refreshDashboard(manual=false){if(refreshing)return refreshing;refreshButton.classList.add('spinning');refreshButton.disabled=true;refreshCopy.textContent=manual?'更新中…':'同步中…';refreshing=(async()=>{try{const response=await fetch('/dashboard/data',{headers:{accept:'application/json'},cache:'no-store'});if(response.status===401){location.href='/dashboard';return;}const data=await response.json();if(!response.ok)throw new Error(data.error||('HTTP '+response.status));updateStaticState(data);if(data.revision!==flow.dataset.revision){flow.innerHTML=data.sectionsHtml||'<div class="empty">目前沒有可顯示的 playlist 影片。</div>';flow.dataset.revision=data.revision;refreshDynamicRefs();apply();updateRelativeTimes();}refreshCopy.textContent='已同步';setTimeout(()=>{if(!refreshing)refreshCopy.textContent='自動更新中';},1400);}catch(error){refreshCopy.textContent='更新失敗';console.error('Dashboard refresh failed',error);}finally{refreshButton.classList.remove('spinning');refreshButton.disabled=false;refreshing=null;}})();return refreshing;}
  ...
  filters.forEach(button=>button.addEventListener('click',()=>chooseFilter(button.dataset.filter||'all')));statButtons.forEach(button=>button.addEventListener('click',()=>chooseFilter(button.dataset.filterTarget||'all',true)));search.addEventListener('input',()=>{apply();saveView();});flow.addEventListener('click',event=>{const toggle=event.target.closest?.('.section-toggle');if(!toggle||!archive?.contains(toggle))return;archiveOpen=archive.classList.contains('collapsed');syncArchive();saveView();});window.addEventListener('pagehide',saveView);
  refreshButton.addEventListener('click',()=>{void refreshDashboard(true);});setInterval(()=>{void refreshDashboard(false);},60000);setInterval(updateRelativeTimes,30000);
...
export async function handleDashboardRequest(request: Request, env: Env): Promise<Response | null> {
  ...
  if (request.method === "GET" && url.pathname === "/dashboard/data") {
    if (!(await dashboardAuthorized(request, env))) return jsonResponse({ error: "unauthorized" }, 401);
    try {
      const { rows, manifest } = await loadDashboardState(env);
      return jsonResponse(dashboardData(env, rows, manifest));
    } catch (error) {
      return jsonResponse({ error: error instanceof Error ? error.message : String(error) }, 500);
    }
  }
  ...
  const { rows, manifest } = await loadDashboardState(env);
  return htmlResponse(renderDashboard(env, rows, manifest));
}
