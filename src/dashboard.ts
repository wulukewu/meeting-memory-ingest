...
function renderDashboard(env: Env, rows: DashboardRow[], manifest: Manifest): string {
  ...
  <section class="attention-strip${attentionCount > 0 ? " warn" : ""}"><div class="attention-icon">${attentionCount > 0 ? "!" : "✓"}</div><div class="attention-copy"><strong>${escapeHtml(attentionText)}</strong><span>${escapeHtml(attentionDetail)}</span></div></section>
  ...
  <div class="flow" id="list">${sections || '<div class="empty">目前沒有可顯示的 playlist 影片。</div>'}</div></main><script>
  const filters=[...document.querySelectorAll('.filter')],statButtons=[...document.querySelectorAll('[data-filter-target]')],cards=[...document.querySelectorAll('.video-card')],sections=[...document.querySelectorAll('.flow-section')],search=document.getElementById('search'),runButton=document.getElementById('run-now'),refreshButton=document.getElementById('refresh-now'),refreshCopy=document.getElementById('refresh-copy'),runStatus=document.getElementById('run-status'),archive=document.querySelector('[data-section="archive"]'),archiveToggle=archive?.querySelector('.section-toggle');
  const stateKey='meeting-memory-dashboard-view-v2';let active='all',archiveOpen=false,saved={};try{saved=JSON.parse(sessionStorage.getItem(stateKey)||'{}')||{};}catch{}
  ...
  function syncArchive(forceOpen=false){if(!archive)return;const shouldOpen=forceOpen||archiveOpen||active==='private'||Boolean((search.value||'').trim());archive.classList.toggle('collapsed',!shouldOpen);archiveToggle?.setAttribute('aria-expanded',String(shouldOpen));if(archiveToggle)archiveToggle.textContent=shouldOpen?'收起':'展開';}
